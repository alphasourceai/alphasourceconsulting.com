...
const moduleShortcuts = [
  { icon: "users", label: "Clients", metric: "128", meta: "24 active this month", tone: "info" },
  { icon: "pen", label: "Agreements", metric: "9", meta: "2 need countersignature", tone: "lilac" },
  { icon: "credit", label: "Billing", metric: "$18.4k", meta: "open checkout value", tone: "success" },
  { icon: "lock", label: "Secure Uploads", metric: "17", meta: "private files pending review", tone: "warning" },
  { icon: "activity", label: "Analysis", metric: "31", meta: "published results", tone: "info" },
  { icon: "file", label: "PDF Reports", metric: "6", meta: "ready to generate", tone: "neutral" },
] as const;
  ...
  return (
    <section className="rounded-lg border border-[#0A1547]/10 bg-white px-5 py-4 shadow-[0_12px_28px_rgba(10,21,71,0.05)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex min-w-64 items-center gap-2 rounded-lg border border-[#0A1547]/10 bg-[#F8F9FD] px-3 py-2 text-sm font-semibold text-[#0A1547]/55">
            <Icon name="search" />
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-sm font-black text-[#0A1547]">{item.label}</p>
                <p className="mt-1 text-xs font-semibold text-[#0A1547]/54">{item.meta}</p>
              </div>
            </div>
            <p className="shrink-0 text-xl font-black text-[#0A1547]">{item.metric}</p>
          </div>
        </article>
      ))}
    </section>
    ...
    <section className="rounded-lg border border-[#0A1547]/10 bg-white shadow-[0_12px_28px_rgba(10,21,71,0.05)]">
      <div className="flex flex-col gap-4 border-b border-[#0A1547]/10 p-5 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 truncate text-sm font-semibold text-[#0A1547]/58">morgan@brightpath.example</p>
        </div>
      ...
      <dl className="grid gap-x-6 gap-y-4 p-5 sm:grid-cols-2 xl:grid-cols-3">
        {clientFacts.map((fact) => (
          <div key={fact.label} className="min-w-0">
            <dt className="text-[11px] font-extrabold uppercase tracking-[0.12em] text-[#0A1547]/38">{fact.label}</dt>
            <dd className="mt-1 truncate text-sm font-bold text-[#0A1547]">{fact.value}</dd>
          </div>
        ))}
      </dl>
    </section>
...
function BillingCards() {
  return (
    <section className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
      <MetricCard icon="credit" label="Open balance" value="$2,400" status="Checkout sent" tone="info" />
      <MetricCard icon="check" label="Paid uploads" value="4" status="All protected" tone="success" />
      <MetricCard icon="clock" label="Open links" value="2" status="Expires soon" tone="warning" />
    </section>
  );
}
  ...
  return (
    <section className="rounded-lg border border-[#0A1547]/10 bg-white p-4 shadow-[0_10px_24px_rgba(10,21,71,0.04)]">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-black text-[#0A1547]">Agreement status</p>
          <p className="mt-1 text-xs font-semibold text-[#0A1547]/54">BAA/Privacy Agreement</p>
        </div>
    ...
    <section className="grid gap-4">
      ...
      <section className="rounded-lg border border-[#0A1547]/10 bg-white p-5 shadow-[0_12px_28px_rgba(10,21,71,0.05)]">
        ...
        <details className="mt-4 rounded-lg border border-[#0A1547]/10 bg-[#F8F9FD] p-4">
          <summary className="admin-focus cursor-pointer text-sm font-extrabold text-[#0A1547]">
            View metadata
          </summary>
          <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
            {detailRows.map((row) => (
              <div key={row.label} className="min-w-0">
                <dt className="text-[11px] font-extrabold uppercase tracking-[0.12em] text-[#0A1547]/38">{row.label}</dt>
                <dd className="mt-1 truncate font-semibold text-[#0A1547]/72">{row.value}</dd>
              </div>
            ))}
          </dl>
        </details>
  ...
  return (
    <article className="grid gap-3 p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="truncate text-sm font-black text-[#0A1547]">{title}</p>
          <p className="mt-1 truncate text-xs font-semibold text-[#0A1547]/54">{subtitle}</p>
        </div>
      </div>
      <div className="flex items-center gap-2 md:justify-end">
        <span className="hidden text-xs font-bold text-[#0A1547]/45 sm:inline">{meta}</span>
...
function MetricCard({
  ...
  return (
    <article className="rounded-lg border border-[#0A1547]/10 bg-white p-4 shadow-[0_10px_24px_rgba(10,21,71,0.04)]">
      ...
      <p className="mt-4 text-[11px] font-extrabold uppercase tracking-[0.12em] text-[#0A1547]/38">{label}</p>
      <p className="mt-1 text-2xl font-black text-[#0A1547]">{value}</p>
    </article>
  );
}
  ...
  return (
    <div className="grid grid-cols-[24px_minmax(0,1fr)] gap-3">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-0.5 truncate text-xs font-semibold text-[#0A1547]/52">{value}</p>
      </div>
    </div>
  );
}
...
function StateTile({ body, icon, title, tone }: { body: string; icon: IconName; title: string; tone: StatusTone }) {
  return (
    <div className="rounded-lg border border-[#0A1547]/10 bg-white p-4 text-center shadow-[0_10px_24px_rgba(10,21,71,0.04)]">
      ...
      <p className="mt-3 text-sm font-black text-[#0A1547]">{title}</p>
      <p className="mt-1 text-xs font-semibold leading-5 text-[#0A1547]/54">{body}</p>
    </div>
  );
}
...
function MenuButton({ icon, label }: { icon: IconName; label: string }) {
  return (
    <button
      type="button"
      className="admin-focus flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-sm font-bold text-[#0A1547]/72 transition hover:bg-[#F8F9FD] hover:text-[#0A1547]"
    >
...
function IconButton({
  ...
  const className = variant === "primary"
    ? "admin-focus inline-flex h-10 items-center gap-2 rounded-lg bg-[#0A1547] px-3 text-sm font-extrabold text-white transition hover:bg-[#1A2460]"
    : "admin-focus inline-flex h-10 items-center gap-2 rounded-lg border border-[#0A1547]/10 bg-white px-3 text-sm font-extrabold text-[#0A1547] transition hover:border-[#A380F6]/50";
...
function IconBadge({ icon, tone }: { icon: IconName; tone: StatusTone }) {
  return (
    <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${badgeToneClassName(tone)}`}>
      <Icon name={icon} />
    </span>
  );
}
...
function StatusChip({ label, tone }: { label: string; tone: StatusTone }) {
  return (
    <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-extrabold ${chipToneClassName(tone)}`}>
      {label}
    </span>
  );
}
...
function chipToneClassName(tone: StatusTone): string {
  switch (tone) {
    ...
    case "info":
      return "border-[#02ABE0]/30 bg-[#02ABE0]/10 text-[#0A1547]";
    case "warning":
    ...
    case "neutral":
    default:
      return "border-[#0A1547]/10 bg-[#F8F9FD] text-[#0A1547]/62";
  }
}

function badgeToneClassName(tone: StatusTone): string {
  switch (tone) {
    case "success":
      return "bg-[#02D99D]/14 text-[#0A1547]";
    case "info":
