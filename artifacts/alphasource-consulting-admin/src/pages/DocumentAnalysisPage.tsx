...
type ConsultantReviewExportOptions = Record<ConsultantReviewExportSection, boolean>;
...
const consultantReviewExportDefaults: ConsultantReviewExportOptions = {
...
const consultantReviewExportSections: Array<{ key: ConsultantReviewExportSection; label: string }> = [
...
function ConsultantReviewPanel({
  ...
  function exportConsultantReview() {
    ...
    if (!printWindow) {
      setExportError("Unable to open the export window. Check popup settings and try again.");
      return;
    }

    const html = buildConsultantReviewExportHtml({
      analysisData,
    ...
    <section className="admin-card mt-5 p-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          >
            Export consultant review
          </button>
        </div>
      </div>

      {exportOpen && (
        <div className="mt-5 rounded-2xl border border-[#A380F6]/20 bg-[#F8F9FD] p-4">
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
            <button
              ...
              className="admin-focus rounded-xl border border-[#0A1547]/10 bg-white px-4 py-2 text-sm font-extrabold text-[#0A1547] transition hover:bg-[#F8F9FD]"
            >
              Cancel
            </button>
            <button
...
type ConsultantReviewExportInput = {
  analysisData: AdminAnalysisData;
...
<body>
  <main class="page">
    ...
    <div class="meta">
      ${consultantReviewMetaItemHtml("Tool", formatStructuredLabel(structuredAnalysis.toolType || analysisData.toolType))}
      ${consultantReviewMetaItemHtml("Source format", formatStructuredLabel(analysisData.sourceFormat || "processed"))}
      ${consultantReviewMetaItemHtml("Processed at", formatDate(analysisData.generatedAt || null))}
